...
eng = pd.read_csv('../Sentiment_Youtube_Data/fr_comm.csv')
...
weekly_polarity_dataset = pd.DataFrame({
    ...
    'Mean_Polarity_YTB': weekly_polarity.values      # Use the mean polarity values
})
# Convert 'date' column to datetime
...
result = seasonal_decompose(weekly_polarity_dataset['Mean_Polarity_YTB'], model='additive', period=20)
...
weekly_polarity_dataset['Polarity_Trend_YTB'] = result.trend.values
...
trend_results = intervals('Polarity_Trend_YTB')

# Create three new columns based on the result
merged_df['Sen_I_Ytb'] = 0
merged_df['Sen_D_Ytb'] = 0
merged_df['Sen_S_Ytb'] = 0

for start_week, end_week, trend in trend_results:
    if trend == 'Increasing':
        merged_df.loc[start_week:end_week, 'Sen_I_Ytb'] = 1
    elif trend == 'Decreasing':
        merged_df.loc[start_week:end_week, 'Sen_D_Ytb'] = 1
    else:
        merged_df.loc[start_week:end_week, 'Sen_S_Ytb'] = 1
########################
...
eg = pd.read_csv('../Sentiment_Twitter_Data/TwitterData_Class_FR.csv')
# Convert 'date' column to datetime
eg['Date'] = pd.to_datetime(eg['Date'])
# Sort the DataFrame by the 'date' column
eg = eg.sort_values(by='Date')

# Resample the data on a weekly basis and calculate mean polarity for each week
weekly_polarity = eg.resample('W-Mon', on='Date')['Polarity'].mean()
weekly_polarity.fillna(0, inplace=True)
...
# merge the two datasets
weekly_polarity_dataset['Week_Number'] = weekly_polarity_dataset['Week_Number'].dt.tz_localize(None)
merged_df = pd.merge(weekly_polarity_dataset, merged_df, on='Week_Number', how='right')

# Apply the intervals function to merged_df
trend_results = intervals('Polarity_Trend')

# Create three new columns based on the result
merged_df['Sen_I_Tw'] = 0
merged_df['Sen_D_Tw'] = 0
merged_df['Sen_S_Tw'] = 0

for start_week, end_week, trend in trend_results:
    if trend == 'Increasing':
        merged_df.loc[start_week:end_week, 'Sen_I_Tw'] = 1
    elif trend == 'Decreasing':
        merged_df.loc[start_week:end_week, 'Sen_D_Tw'] = 1
    else:
        merged_df.loc[start_week:end_week, 'Sen_S_Tw'] = 1
...
ax2.plot(merged_df['Polarity_Trend_YTB'], label='YTB_polarity', color='black')
ax2.plot(merged_df['Polarity_Trend'], label='TWT_polarity', color='yellow')
...
print(merged_df.head(50))
merged_df.to_csv('Csvs/Yb&Tw_FRcontext.csv')
